...
import type { AgentType } from "./loader.server"
interface AgentProp {
  agents: AgentType[]
}
export default function UserList({agents}: AgentProp){
  // console.log(agents)
  return(
    <>
      <div className="mt-6 mb-8 flex flex-col gap-12">
        <div className="relative flex flex-col bg-clip-border rounded-xl bg-white text-gray-700 shadow-md">
          <div className="relative bg-clip-border mx-4 rounded-xl overflow-hidden bg-gradient-to-tr from-gray-900 to-gray-800 text-white shadow-gray-900/20 shadow-lg -mt-6 mb-8 p-6">
            <h6 className="block antialiased tracking-normal font-sans text-base font-semibold leading-relaxed text-white">Agent</h6>
          </div>
            ...
            <table className="w-full min-w-[640px] table-auto">
              ...
              <tbody>
                {agents && agents.map((user,index) => (
                    <tr key={user.id}>
                      <td className={`py-3 px-5 ${index !== agents.length - 1 ? "border-b border-blue-gray-50" : ""}`}>
                      ...
                      <td className={`py-3 px-5 ${index !== agents.length - 1 ? "border-b border-blue-gray-50" : ""}`}>
                        <div className="flex items-center gap-4">
                          <Link to={`/admin/user/${user.email}/edit?utb=agent`} className="block antialiased font-sans text-xs font-semibold text-blue-gray-600">Edit</Link>
                        </div>
                      </td>
                    </tr>
                ))}
